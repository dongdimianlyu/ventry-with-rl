...
export function TeamTaskCard({ task, onToggleComplete }: TeamTaskCardProps) {
  ...
  return (
    <>
      ...
      <Card 
        className={`relative min-w-[320px] max-w-[380px] ${task.businessImpact ? 'h-[320px]' : 'h-[280px]'} flex flex-col ${task.completed ? 'opacity-60' : ''} ${config.cardBorder} ${config.cardBackground} border rounded-xl hover:shadow-lg transition-all duration-200 hover:-translate-y-1 cursor-pointer`}
                  ...
                  <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 rounded-xl p-6">
                    <div className="flex items-start space-x-4">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-base text-blue-800 leading-relaxed">{task.reason}</p>
                      </div>
                    </div>
                  </div>
                )}
